...
if(isset($_SESSION["authentification"])){
    ...
    include("../modules/Certificat.class.php");
    ...
    if((!$_REQUEST['op'])||($_REQUEST['op']>70))	{header("Location: ../index.php?pg=tableau-de-bord");}
        ...
        if($_GET['op']==3){
            ...
            $fiche = new Certificat();

            $client = new Accounts();
            $client->getFromDB($_REQUEST["id_client"]);
            if (!file_exists('../certificat/'.$client->getMatricule().'')) {
                mkdir('../certificat/'.$client->getMatricule().'', 0777, true);
            }
            $dest_dossier = "../certificat/".$client->getMatricule()."/";
            chmod($dest_dossier,0777);
            include("_uploadPhoto.php");
            if($fichier == "d41d8cd98f00b204e9800998ecf8427e."){$fichier = NULL;}


            $fiche->setCertificat(NULL,$_REQUEST["id_client"], $_REQUEST["mois"], $_REQUEST["annee"], $fichier,date("Y-m-d H:i:s"),$_REQUEST["type"]);
           ...
           $client = new Accounts();
            ...
            $fiche->setCertificat($_REQUEST["id"], $_REQUEST["id_client"], $_REQUEST["mois"], $_REQUEST["annee"], $image,$_REQUEST['date_creation'],$_REQUEST["type"]);
