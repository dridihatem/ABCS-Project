...
class Certificat extends DB{
    ...
    public function setCertificat($id,$id_client,$mois,$annee,$pdf,$date_creation,$type)
    {
        $this->id = $id;
    ...
    public function updateToDB(){
        $query = "UPDATE ".$_SESSION['pfx']."_certificat SET ";
        $query .= $this->getSQL();
    ...
    public function saveToDB(){
        $query = "INSERT INTO ".$_SESSION['pfx']."_certificat SET ";
        $query .= $this->getSQL();
        return $this->connect()->query($query);
    }
    public function deleteFromDB($id){
        $query = "DELETE From ".$_SESSION['pfx']."_certificat ";
  ...
  public function deleteFichierFromDB($id)
    {
        ...
        $query = "update " . $_SESSION['pfx'] . "_certificat set pdf = ''";
    ...
    {
        $res = $this->connect()->query(sprintf("SELECT * FROM " . $_SESSION['pfx'] . "_certificat WHERE id = '%d'", mysqli_real_escape_string($this->connect(), $id)));
    ...
    {
        $res = $this->connect()->query("SELECT COUNT(*) AS total FROM " . $_SESSION['pfx'] . "_certificat WHERE type=2");
       $row = $res->fetch_array();
        echo $row["total"];
    }
